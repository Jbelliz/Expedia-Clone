...
export const addHotel = (payload) => (dispatch) => {
  ...
  axios
    .post(`${API_URL}/hotel`, payload) // https://makemytrip-api-data.onrender.com/hotel
    .then(() => {
...
export const fetchingHotels = (limit) => (dispatch) => {
  axios
    .get(`${API_URL}/hotel?_limit=${limit}`) // https://makemytrip-api-data.onrender.com/hotel?_limit=${limit}
    .then((res) => {
...
export const DeleteHotel = (deleteId) => async (dispatch) => {
  try {
    const res = await fetch(
      `${API_URL}/hotel/${deleteId}`, // https://makemytrip-api-data.onrender.com/hotel/${deleteId}
      {
        method: "DELETE",
        headers: {
